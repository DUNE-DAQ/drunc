...
class K8sProcessManager(ProcessManager):
    def __init__(self, configuration, **kwargs):
        ...
        applications running in Pods. It leverages the Kubernetes API for service discovery,
        which is a more scalable approach than creating a service for each pod.
        """
        ...
        self._rbac_v1_api = client.RbacAuthorizationV1Api()
    ...
    def _create_namespace(self, session):
        """Creates a Kubernetes namespace and the necessary RBAC resources if they don't already exist."""
        ...
        try:
            ...
            if e.status == 404:
                ...
                # Create RBAC resources for pod discovery
                service_account_name = f"{session}-discover"
                role_name = f"{session}-pod-reader"
                role_binding_name = f"{session}-read-pods"

                # Create ServiceAccount
                sa = client.V1ServiceAccount(
                    metadata=self._meta_v1_api(name=service_account_name)
                )
                self._core_v1_api.create_namespaced_service_account(
                    namespace=session, body=sa
                )

                # Create Role
                role = client.V1Role(
                    metadata=self._meta_v1_api(name=role_name),
                    rules=[
                        client.V1PolicyRule(
                            api_groups=[""],
                            resources=["pods"],
                ...
                self._rbac_v1_api.create_namespaced_role(namespace=session, body=role)

                # Create RoleBinding
                rb = client.V1RoleBinding(
                    metadata=self._meta_v1_api(name=role_binding_name),
                    subjects=[
                        RbacV1Subject(
                            kind="ServiceAccount",
                ...
                self._rbac_v1_api.create_namespaced_role_binding(
                    namespace=session, body=rb
                )
            else:
                raise e
