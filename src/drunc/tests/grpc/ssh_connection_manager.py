...
from drunc.tests.grpc.remote_cli_command_builder import BootServerCommand
...
class SSHConnectionManager(ProcessConnectionManager):
    ...
    def __init__(self, 
                 boot_commands: Dict[str, BootServerCommand],
        ...
        print(f"SSH connection manager initialised with {len(boot_commands)} boot commands:")
        for server_id, boot_cmd in boot_commands.items():
    ...
    def start_process(self, handle: RunningGrpcServer) -> None:
        ...
        try:
            ...
            # Execute the complete pre-built SSH command
            ssh_process = self.ssh(
                *boot_command.complete_ssh_command[1:],  # Skip '/usr/bin/ssh' part
                _out=str(log_file),
                _err=str(log_file),
                _bg=True,
                _bg_exc=False,
            ...
            # Brief delay to catch immediate SSH connection failures
            time.sleep(0.2)
            
            # Check if SSH process failed immediately
            if not ssh_process.is_alive():
                error_output = self._read_recent_log_output(log_file)
                raise RuntimeError(
                    f"SSH process for {handle.process_id} failed immediately. "
                    f"Command: {' '.join(boot_command.complete_ssh_command)}. "
                    f"Log output: {error_output}"
                )
    ...
    def _start_process_watcher(self, handle: RunningGrpcServer, boot_command: BootServerCommand) -> None:
        """
        ...
        def watch_process():
            ...
            try:
                handle.process.wait()
                exit_code = handle.process.exit_code if hasattr(handle.process, 'exit_code') else None
                ...
                if exit_code == 0:
                    ...
                    print(f"SSH process {handle.process_id} exited with code 255 (remote process may still be running)")
                else:
                    # Read log output for error details
                    log_file = self.log_directory / f"{handle.process_id}.log"
                    error_output = self._read_recent_log_output(log_file, 50)
                ...
                error_msg = f"SSH process {handle.process_id} terminated with exception: {e}"
                print(f"ERROR: {error_msg}")
                handle.startup_error = error_msg
                
        watcher = threading.Thread(
            target=watch_process,
            name=f"SSHWatcher-{handle.process_id}",
            daemon=True
        )
        watcher.start()
    ...
    def get_process_startup_error(self, handle: RunningGrpcServer) -> Optional[str]:
        ...
        Args:
            handle: ProcessHandle to check for startup errors
