...
def match_children(statuses: list, descriptions: list) -> list:
    ...
    if len(descriptions) != len(children):
        raise MalformedCommand(
            f"Command {inspect.currentframe().f_code.co_name} has assigned the incorrect number of children!"
        )
    return children
...
def print_status_table(obj, statuses: DecodedResponse, descriptions: DecodedResponse):
    ...
    if type(statuses.data) != Status:
        data_type = (
            statuses.data.TypeName()
            if type(statuses.data) == Any
            else type(statuses.data)
        )
        log = logging.getLogger("controller.shell_utils")
        log.warning(
            f"Could not get the status of the controller, got a '{data_type}' instead"
        )
        return
...
def controller_setup(ctx, controller_address):
    ...
    with Progress(
        ...
        waiting = progress.add_task(
            "[yellow]Trying to talk to the top controller...", total=timeout
        )
