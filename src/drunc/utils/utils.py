...
def root_logger_is_setup(log_level: int) -> bool:
    if "drunc" in logging.Logger.manager.loggerDict:
        ...
        if root_logger.level == log_levels["NOTSET"] and log_level != "NOTSET":
            ...
            root_logger.debug(
                f'Root logger level updated from "NOTSET" to {logging.getLevelName(log_level)}'
            )
        return True
    return False
...
def get_logger(
    ...
    override_log_file: bool = False,
    rich_handler: bool = False,
):
    logger_dict = logging.Logger.manager.loggerDict
    if "drunc" not in logger_dict:
        raise DruncSetupException(
            "Required root logger 'drunc' has not been initialized"
        )
    if logger_name == "":
        raise DruncSetupException(
            "This was an attempt to set up the root logger `drunc`, need to run `setup_root_logger` first."
        )
    if logger_name.split(".")[0] == "drunc":
        raise DruncSetupException(
            f"get_logger adds the root logger prefix, it is not required for {logger_name}"
        )
    if override_log_file and not log_file_path:
        raise DruncSetupException(
            "Configuration error - a log_file_path must be provided if it is to be overwritten"
        )
    if logger_name.count(".") > 2:
        raise DruncSetupException(
            f"Logger {logger_name} has a larger inheritance structure than allowed."
        )
    if logger_name == "process_manager" and "drunc.process_manager" not in logger_dict:
        if not log_file_path:
            raise DruncSetupException(
                "process_manager logger setup requires a log path."
            )
        if not rich_handler:
    ...
    if ("drunc." + logger_name) in logger_dict:
        function_logger.debug(
            "This logger has already been set up, returning the original"
        )
        logger = logging.getLogger("drunc." + logger_name)
        return logger
    if (
        logger_name.count(".") == 2
        and ("drunc." + logger_name.split(".")[0]) not in logger_dict
    ):
        function_logger.debug(
            f"Parent of logger {logger_name} (drunc.{logger_name.split('.')[0]}) not set up yet, setting it up now"
        )
        get_logger(
            logger_name.split(".")[0], log_file_path, override_log_file, rich_handler
        )

    logger_level = logging.getLogger("drunc").level
    if not logger_level:
        raise DruncSetupException(
            f"Root logger level not set (found level {logging.getLevelName(logger_level)})"
        )
    ...
    if any(
        ...
        function_logger.debug(
            f"Logger {logger_name} already has an associated and usable RichHandler, skipping it"
        )
        stdHandler = None
    elif rich_handler:
        ...
        function_logger.debug(
            f"Logger {logger_name} already has an associated and usable StreamHandler, skipping it"
        )
        stdHandler = None
    else:
...
def print_traceback(e):  # RETURNTOME - rename to print_console_traceback
    log = get_logger(logger_name="utils.traceback", rich_handler=True)
    log.exception(e)
