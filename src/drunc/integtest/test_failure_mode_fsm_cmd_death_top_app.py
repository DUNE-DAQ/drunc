...
def test_fsm_cmd_application_death_ps_table(run_dunerc) -> None:
    ...
    top_segment_application_row = get_rows_for_friendly_name(
        ps_table_post_conf, "ft-top-segment-application"
    )
    assert top_segment_application_row, (
        "Expected to find a row for the nested segment 2 application in the ps table, but did not."
    )
    assert top_segment_application_row[0]["alive"] == "False", (
        "Expected top segment application to be dead after fsm cmd execution, but found it alive."
    )
...
def test_session_in_error_cli(run_dunerc) -> None:
    ...
    # Check that the application is marked as disconnected in the status table
    top_segment_application_row = get_rows_for_status_name(
        status_table_post_conf, "ft-top-segment-application"
    )
    assert top_segment_application_row, (
        "Expected to find a row for the top segment application in the status table, but did not."
    )
    assert top_segment_application_row[0]["State"] == "disconnected", (
        f"Expected top segment application state to be 'disconnected', but found '{top_segment_application_row[0]['State']}'."
    )
