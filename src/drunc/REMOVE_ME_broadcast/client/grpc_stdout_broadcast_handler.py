...
class gRPCStdoutBroadcastHandler(BroadcastReceiverServicer):
    def __init__(self, conf: ControllerConfHandler, token, **kwargs) -> None:
        super(gRPCStdoutBroadcastHandler, self).__init__(**kwargs)
        from drunc.exceptions import DruncSetupException

        raise DruncSetupException(
            "gRPCStdoutBroadcastHandler is not handled it needs to be reworked!"
        )
        self.ready = False
        self.stub = None
        self.token = token
    ...
    def disconnect(self) -> None:
        ...
        try:
            ...
            self._log.error(
                "Could not disconnect from broadcaster (maybe it' dead and you won't receive any broadcast anyway)"
            )
            self._log.error(str(e))
